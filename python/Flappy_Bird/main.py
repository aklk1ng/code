...
punch_sound = pygame.mixer.Sound('music/punch.mp3')
...
class BirdSprite(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.bird_image = pygame.image.load('image/bird.png')
    ...
    def collie(self, pipe: 'Pipe'):
        return (
            pygame.sprite.collide_mask(self, pipe.top) or pygame.sprite.collide_mask(self, pipe.bot)
        )

class PipeSprite(EdgesMixin, pygame.sprite.Sprite):
    def __init__(self, side: str, padding: int, from_right: int = 0):
        super().__init__()
        self.image = pygame.image.load(f"imges/{side}_pipe.png")
        self.image.set_colorkey(WHITE)
        self.image.convert_alpha()
        self.rect = self.image.get_rect()

        width = WHITE * self.w // 2 - from_right
        if side == 'top':
...
while True:
    ...
    if game_on or init:
        ...
        for pipe in pipes:
            if (pipe.top.y + pipe.top.h < 100 or pipe.bot.y > HEIGHT - 100):
                pipe.y_delta *= -1
            pipe.top.rect.move_ip(-5, pipe.y_delta)
            pipe.bot.rect.move_ip(-5, pipe.y_delta)
            pipe.top.draw(display)
            pipe.bot.draw(display)
        if pipe[0].top.x + pipe[0].top.w < 0:
            pipes.pop(0)

        if not init:
            if pressed[pygame.locals.K_SPACE] and not jump_freeze:
                jump_sound.play()
                bird.velocity = JUMP_VELOCITY
            bird.velocity -= GRAVITY
            prev_y = bird.y
            bird.rect.move_ip(9, -bird.velocity)
            y_diff = prev_y - bird.y
            rot = (max(min(y_diff, JUMP_VELOCITY), -JUMP_VELOCITY + JUMP_VELOCITY)) / (JUMP_VELOCITY * 2)
            new_degree = 160 * rot - 70
            degree_diff = math.dist((bird.degree, ), (new_degree, ))
            new_degree = bird.degree + math.copysign(min(40, abs(degree_diff)), new_degree - bird.degree)
            bird.image = pygame.transform.rotate(bird.bird_image, new_degree)
            bird.degree = new_degree

            if bird.y <= 0 or bird.y >= HEIGHT or bird.collie(pipes[0]):
                punch_sound.play()
                game_on = False
    ...
    if pressed[pygame.locals.K_SPACE] and not game_on:
        game_on = True
        bird = BirdSprite()
        if len(pipes) != 1:
            pipes = [create_pipe(400)]
    ...
    for event in pygame.event.get():
        ...
        elif event.type == pygame.locals.KEYDOWN and event.type == pygame.locals.K_SPACE:
            jump_freeze = False
