...
inline ll work(ll L, ll R, ll l, ll r) {
    ...
    for (ll i = L; i < R; i++) {
        ll x = (s[i] == '-') ? 0 : (s[i] - 'a' + 1);
        a[0][i - L] = x ? 0 : 1, a[1][i - L] = 2 * x, a[2][i - L] = sqr(x),
                 d[i] = sqr(x);
    }
    d[R] = 0;
    for (ll i = l; i < r; i++) {
        ll x = (t[i] == '-') ? 0 : (t[i] - 'a' + 1);
        b[0][r - i] = sqr(x), b[1][r - i] = x, b[2][r - i] = x ? 0 : 1,
                 z += sqr(x);
    }
    for (ll f = 0; f < 3; f++) {
        dft(M, a[f], 0), dft(M, b[f], 0);
        for (ll i = 0; i < M; i++) {
            c[i] = (c[i] + a[f][i] * b[f][i]) % Mod;
        }
    }
    dft(M, c, 1);
    for (ll i = 0; i < r - l; i++) {
    ...
    for (ll i = L; i <= R - (r - l); z -= d[i], z += d[i + (r - l)], i++) {
        if (z % Mod == c[i - L + r - l]) {
            return i;
        }
    }
...
void solve() {
    for (ll i = 1; i < N; i <<= 1)
        g[i] = power(G, Mod / 4 / i), iv[i] = power(i, Mod - 2);
    std::cin >> n >> m;
    std::cin >> s >> t;
