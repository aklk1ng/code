...
typedef struct ThreadPool
{
    ...
    int shutdown;// the flag of whether deatory the thread pool，1 is destroy，0 is not destory
...
void* manager(void* arg)
{
    ...
    while (!pool->shutdown) {
        ...
        // get the number of thread's task(queueSize) ad the current idle thread(livenum)
        pthread_mutex_lock(&pool->mutexPool);
        ...
        // destroy thread(the busynum * 2 < livenum && livenum > minnum)销毁线程--忙的线程*2 < 存活的线程数 && 存活的线程数 < 最小线程数
